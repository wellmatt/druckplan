        ...
        success: function(data) {
            if(data != '0') {
                ...
                jqcc.each(data, function(type, item) {
                    temp = temp + '<div class="chat" id="'+item.id+'|'+item.previd+'" title="<?php echo $chathistory_language[8];?>"><div class="chatrequest"><b>'+item.from+'</b></div><div class="chatmessage chatmessage_short">'+item.message+'</div><div class="chattime" timestamp='+item.sent+'></div><div style="clear:both"></div></div>';                                                 });           
...
function getMessage(id, range,chatroommode, basedata, lastmessageid) {
	...
	jqcc.ajax({
            ...
            success: function(data) {
                ...
                jqcc('div.chatbar_1').html('<?php echo $chathistory_language[2];?> '+ name +' <?php echo $chathistory_language[4];?> <small class="chattimedate" timestamp="'+time+'"></small> (<span id="messageCount">'+count+'</span> <?php echo $chathistory_language[3];?>)');
                jqcc('.chattime').each(function(key,value) {
                var ts = new Date(jqcc(this).attr('timestamp') * 1000);
                var timest = getTimeDisplay(ts);
                jqcc(this).html(timest);
                ...
                jqcc('small.chattimedate').each(function(key,value){
                    var ts = new Date(jqcc(this).attr('timestamp') * 1000);
