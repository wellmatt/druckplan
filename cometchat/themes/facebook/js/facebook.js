        ...
        return {
            ...
            initialize: function(){
                ...
                if(settings.showOnlineTab==1){
                    ...
                    usertabpop = '<div id="cometchat_userstab_popup" class="cometchat_tabpopup"><div class="cometchat_userstabtitle"><div class="cometchat_userstabtitletext">'+language[12]+'</div><div class="cometchat_minimizebox cometchat_tooltip" id="cometchat_minimize_userstab_popup" data-title="Minimize User Tab"></div><br clear="all"/></div>'+trayData+'<div class="cometchat_tabcontent cometchat_tabstyle"><div id="cometchat_userscontent"><div id="cometchat_userslist"><div class="cometchat_nofriends">'+language[41]+'</div></div></div></div></div>';
                }
            ...
            createChatboxData: function(id, name, status, message, avatar, link, silent, tryOldMessages){
                ...
                shortname = name;
                longname = name;
                $("<span/>").attr("id", "cometchat_user_"+id).attr("amount", 0).addClass("cometchat_tab").html(isMobile+'<div class="cometchat_userscontentdot cometchat_'+status+' cometchat_floatL"></div><div class="cometchat_unreadCount cometchat_floatL">0</div><div class="cometchat_user_shortname">'+shortname+'</div><div class="cometchat_closebox_bottom cometchat_tooltip" data-title="Close Tab" id="cometchat_closebox_bottom_'+id+'">×</div>').appendTo($("#cometchat_chatboxes_wide"));
                var pluginshtml = '';
                var avchathtml = '';
                var smiliehtml = '';
                ...
                $.each(item, function(i, incoming){
                    ...
                    if(jqcc.cometchat.getThemeArray('buddylistName', incoming.from)==null||jqcc.cometchat.getThemeArray('buddylistName', incoming.from)==''){
                        ...
                        if($("#cometchat_message_"+incoming.id).length>0){
                            ...
                            var avatar = baseUrl+"themes/facebook/images/noavatar.png";
                            if(parseInt(incoming.self)==1){
                                msg = '<div class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'"><div class="cometchat_chatboxmessagecontent cometchat_self cometchat_floatR" title="'+sentdata+'">'+incoming.message+'</div><div class="selfMsgArrow"><div class="after"></div></div>';
                                addMessage = 1;

                            }else{
                                msg = '<div class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'">'+selfstyleAvatar+'<div class="cometchat_chatboxmessagecontent cometchat_floatL" title="'+sentdata+'">'+incoming.message+'</div><div class="msgArrow"><div class="after"></div></div></div>';
                                addMessage = 1;
                            }
                ...
                $.each(data, function(type, item){
                    if(type=='messages'){
                        $.each(item, function(i, incoming){
                            ...
                            if(incoming.self==1){
                                $("#cometchat_tabcontenttext_"+id).append('<div class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'"><div class="cometchat_chatboxmessagecontent cometchat_self cometchat_floatR" title="'+jqcc[settings.theme].getTimeDisplay(ts)+'">'+incoming.message+'</div><div class="selfMsgArrow"><div class="after"></div></div></div>');                                
                                jqcc[settings.theme].scrollDown(id);
                               
                            }else{                              
                                $("#cometchat_tabcontenttext_"+id).append('<div class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'">'+selfstyleAvatar+'<div class="cometchat_chatboxmessagecontent cometchat_floatL" title="'+jqcc[settings.theme].getTimeDisplay(ts)+'">'+incoming.message+'</div><div class="msgArrow"><div class="after"></div></div></div>');
                                jqcc[settings.theme].scrollDown(id);                               
                            }
                        });
                    }
