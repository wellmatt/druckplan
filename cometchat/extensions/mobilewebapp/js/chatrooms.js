...
jqcc.extend($.mobilewebapp, {
    ...
    ChatroomMessage: function(id, incomingmessage, self, old, incomingid, selfadded, sent){
        ...
        if(selfadded==0){
            ...
            if($("#cometchat_message_"+self).length>0){
                $("#cometchat_message_"+self).find(".cometchat_chatboxmessagecontent").html(incomingmessage);
            }else{
                sentdata = '';
                if(incomingid!=null){
                    var ts = new Date(incomingid);
                    sentdata = jqcc.mobilewebapp.getTimeDisplay(ts);
                }
                if(!jqcc.cometchat.getChatroomVars('fullName')&&fromname.indexOf(" ")!=-1){
                    fromname = fromname.slice(0, fromname.indexOf(" "));
                }
                $("#currentroom_convotext").append('<div class="cometchat_chatboxmessage me" id="cometchat_message_'+self+'"><span class="cometchat_chatboxmessagefrom"><strong>'+fromname+'</strong>'+separator+'</span><div class="userSeperator"></div><span class="cometchat_chatboxmessagecontent">'+incomingmessage+'</span>'+sentdata+'</div><div style="clear:both;"></div>');
                setTimeout(function(){
                    jqcc.mobilewebapp.crscrollToBottom();
                }, 500);
            }
