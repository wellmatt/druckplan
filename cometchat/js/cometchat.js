...
(function($){
    $.cometchat = $.cometchat||function(){
        ...
        ccvariable.todays12am = ccvariable.todaysDate.getTime()-(ccvariable.todaysDate.getTime()%(24*60*60*1000));
        ...
        }else if(ccvariable.mobileDevice){
        ...
        arguments.callee.chatboxKeydownSet = function(id, message){
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data){
                        if(typeof (jqcc[calleeAPI].addMessages)!=='undefined'){
                            jqcc[calleeAPI].addMessages([{"from": id, "message": data.message, "self": 1, "old": 1, "id": data.insertedid, "sent": Math.floor(new Date().getTime()/1000)}]);
                        }
        ...
        arguments.callee.notify = function(title, image, message, clickEvent, id, msgid){		
            if(navigator.userAgent.match(/chrome/i)&&settings.desktopNotifications==1){	             
