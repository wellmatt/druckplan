...
//$exits = class_exists('CommissionContact');
//$test = realpath('libs/modules/notes/notes.class.php');
...
if ($_REQUEST["subexec"] == "save")
{
    ...
    if($_REQUEST["notes_title"] != NULL && $_REQUEST["notes_title"] != ""){
    	...
    	$note->setModule(Notes::MODULE_COMMISSIONCONTACT);
    	$note->setObjectid($commissionContact->getId());
...
$all_notes = Notes::getAllNotes(Notes::ORDER_TITLE, Notes::MODULE_COMMISSIONCONTACT, $commissionContact->getId());
?>
...
<? /*** Eingabefelder fuer Kommentare anzeigen ****/?>
		...
		<?/*if($commissionContact->getId()){*/?>
				...
				<?/*	foreach ($all_notes AS $n){ */?>
						<tr onmouseover="mark(this, 0)" onmouseout="mark(this,1)">
							...
							<?/* if ($n->getFileName() != "" && $n->getFileName() != NULL){*/?>
								...
								&emsp;
								<a href="#" 
									onclick="askDel('index.php?exec=edit&subexec=deletenotefile&id=<?/*=$commissionContact->getId()*/?>&delnoteid=<?/*=$n->getId()*/?>')"
							...
							<td class="content_row" valign="top">
								...
								<?/*if($_USER->getId() == $n->getCrtuser()->getId()  || $_USER->isAdmin()){*/?>
									<a href="#" 
										onclick="askDel('index.php?exec=edit&subexec=deletenote&id=<?/*=$commissionContact->getId()*/?>&delnoteid=<?/*=$n->getId()*/?>')"
			...
			<table id="table_notes" style="display:none">
				...
				<tr>
					...
					<td class="content_row_clear">
						<textarea name="notes_comment" id="notes_comment" style="width: 482px;height: 150px;"></textarea>
					</td>
				</tr>
				<tr>
					<td class="content_row_header"><?/*=$_LANG->get('Datei-Anhang');*/?></td>
