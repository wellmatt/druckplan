...
$customers = BusinessContact::getAllBusinessContacts(BusinessContact::ORDER_NAME, " (name1 LIKE '%{$_REQUEST['term']}%' OR name2 LIKE '%{$_REQUEST['term']}%') AND customer = 1");

$cust_data = array();

foreach ($customers as $cust) {
	$cust_data[] = array('label'=>$cust->getNameAsLine(),'id'=>$cust->getId(),'value'=>$cust->getNameAsLine()); // getNameAsLine()
	// $cust_data[] = $cust->getNameAsLine();
}
