        ...
        if(count($this->unitsPerHour) > 0)
        {
            ...
            } else if($this->unit == Machine::UNIT_PERHOUR_BOGEN)
            {
                if($machineEntry->getPart() > 0){
                    $time = 60 / ($this->getUnitsPerHour($calc->getPaperCount($machineEntry->getPart())) / $calc->getPaperCount($machineEntry->getPart(),$machineEntry));
                    if ($debug) {
                        echo '$time = 60 / (' . $this->getUnitsPerHour($calc->getPaperCount($machineEntry->getPart())) . ' / ' . $calc->getPaperCount($machineEntry->getPart(),$machineEntry) . ') </br>';
            ...
            foreach ($this->difficulties as $difficulty){
                ...
                if($difficulty["unit"] == self::DIFFICULTY_PAPER)
                {
                    $diff = $this->getDifficultyByValue($machineEntry->getMyPaper()->getId(), $difficulty["id"]);
                    $time = $time * (1 + ($diff / 100));
                }
            }
            if ($debug){
