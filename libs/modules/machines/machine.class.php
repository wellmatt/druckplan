...
class Machine
{
    ...
    private $umschlUmst = 0;		//gln 13.05.2014: Umschlagen/Umstuelpen (nur fÃ¼r OffsetDruck)
    private $lectorId = 0;
    private $maxHours = 0;
...
//                     echo $sql."</br>";
     ...
     * Wenn eine Farbe angegeben ist, wird diese auch berï¿½cksichtigt.
     * 
     * @param Machine $mach
...
//                         echo $sql."</br>";
    ...
    function getRunningTime($machineEntry)
    {
        ...
        // -------------------------------------------------------------------------
        // Maschinenspezifische Zuschlï¿½ge
        ...
        if($machineEntry->getMachine()->getType() == Machine::TYPE_LAGENFALZ)
        {
            ...
            // Laufzeit mal Anzahl Durchgï¿½nge
            $durchgaenge = ceil($anzUsedStations / $this->anzStations);
            $time *= $durchgaenge;
            
            // Rï¿½stzeiten pro Station
        ...
        if($machineEntry->getMachine()->getType() == Machine::TYPE_SAMMELHEFTER)
        {
            ...
            // Seperater Umschlag -> Umschlaganleger rï¿½sten
            if($calc->getPagesEnvelope() > 0 && $this->timeEnvelope > 0)
            {
                $sumFoldSheets--;
            }
        ...
        // END Maschinenspezifische Zuschlï¿½ge
        // -------------------------------------------------------------------------
    ...
    function getMachinePrice($machineEntry)
    {
        ...
        // Aufschlag aus Farbigkeit berï¿½cksichtigen
        if($machineEntry->getPart() == Calculation::PAPER_CONTENT)
