								...
								<tr class="<?= getRowColor($x) ?>" onmouseover="mark(this, 0)"
									...
									<td>
										...
										<select class="form-control" name="invc_supplierid_<?= $x ?>"
												id="invc_supplierid_<?= $x ?>" onfocus="markfield(this,0)"
												onblur="markfield(this,1)">
									...
									<td>
										<input type="checkbox" class="form-control" name="invc_uses_supplier_<?= $x ?>" value="1"
											   onclick="if(this.checked)document.getElementById('invc_supplierid_<?= $x ?>').style.display='';
												   else
									...
									<td>
										<div class="input-group">
											<input type="text" class="form-control" name="invc_taxes_active_<?= $x ?>"
												   id="invc_taxes_active_<?= $x ?>"
												   value="<?= $invoice->getInvc_taxes_active(); ?>"
												   onfocus="markfield(this,0)" onblur="markfield(this,1)"/>
											<span class="input-group-addon">%</span>
										</div>
									</td>
									<td><? if ($invoice->getInvc_price_netto()) {
								...
								<tr class="<?= getRowColor($y) ?>" onmouseover="mark(this, 0)"
									...
									<td>
										...
										<select class="form-control" name="invc_supplierid_<?= $y ?>"
												id="invc_supplierid_<?= $y ?>" onfocus="markfield(this,0)"
												onblur="markfield(this,1)">
									...
									<td>
										<input type="checkbox" class="form-control" name="invc_uses_supplier_<?= $y ?>"
											   value="1"
											   onclick="if(this.checked)document.getElementById('invc_supplierid_<?= $y ?>').style.display='';
												   else
												   document.getElementById('invc_supplierid_<?= $y ?>').style.display='none';">
									</td>
									<td>
										<div class="input-group">
											<input class="form-control" name="invc_price_netto_<?= $y ?>" value=""
												   onfocus="markfield(this,0)" onblur="markfield(this,1)">
											<span
