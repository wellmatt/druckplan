							...
							<table class="table table-hover">
								...
								{  ?>
									<tr class="<?=getRowColor($x)?>" onmouseover="mark(this, 0)"
										...
										<td class="content_row pointer">
											<input type="text" class="form-control" name="invc_price_netto_<?=$x?>" value="<?echo printPrice($invoice->getInvc_price_netto());?> " onfocus="markfield(this,0)" onblur="markfield(this,1)" /> <?=$_USER->getClient()->getCurrency()?></td>
										...
										<td class="content_row pointer"><?if($invoice->getInvc_price_netto()) {echo $invoice->getTaxPrice();echo " ".$_USER->getClient()->getCurrency();}?></td>
										<td class="content_row pointer"><?if($invoice->getInvc_price_netto()) {echo $invoice->getBruttoPrice();echo " ".$_USER->getClient()->getCurrency();}?></td>
