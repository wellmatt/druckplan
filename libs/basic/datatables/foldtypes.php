...
Editor::inst( $db, 'foldtypes' )
    ->fields(
        Field::inst( 'foldtypes.id' )->set(false),
        Field::inst( 'foldtypes.name' )->set(false),
        Field::inst( 'foldtypes.type' )->set(false)
            ->getFormatter( function ( $val, $data, $opts ) {
                $tmpft = new FoldType();
                $types = $tmpft->getTypes();
                foreach ($types as $type) {
                    if ($type['id'] == $val)
                        return $type['name'];
                }
            }),
        Field::inst( 'filestorage.content', 'picture' )->set(false)
            ->getFormatter( function ( $val, $data, $opts ) {
                return '<img src="data:image/jpg;base64,'.base64_encode($val).'">';
            })
    )
    ->leftJoin( 'filestorage', 'filestorage.id', '=', 'foldtypes.imageid' )
    ->process( $_POST )
    ->json();
