    ...
    if (count($_POST["data"]) > 0){
        foreach ($_POST["data"] as $item => $values) {
            if (strpos($item,"row_") !== false){
                ...
                Cachehandler::removeCache($_CONFIG->cookieSecret."_MachineLocktime_".$itemid);
            }
        }
    }
}
...
Editor::inst( $db, 'machines_locktimes' )
    ...
    ->fields(
        ...
        Field::inst( 'start' )
            ...
            ->setFormatter( 'Format::datetime', array( 'from' => 'd.m.Y H:i', 'to' => 'Y-m-d H:i:s' ) ),
        Field::inst( 'end' )
            ->validator( 'Validate::dateFormat', array(
                'empty' => false,
                'format' => 'd.m.Y H:i'
