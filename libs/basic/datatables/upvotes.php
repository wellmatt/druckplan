    ...
    if (count($_POST["data"]) > 0){
        foreach ($_POST["data"] as $item => $values) {
            if (strpos($item,"row_") !== false){
                ...
                Cachehandler::removeCache($_CONFIG->cookieSecret."_UpVote_".$itemid);
            }
        }
    }
}
...
Editor::inst( $db, 'upvotes' )
    ->fields(
        Field::inst( 'upvotes.id' )->set(false),
        Field::inst( 'upvotes.title' )->set(false),
        Field::inst( 'businesscontact.name1' )->set(false),
        Field::inst( 'user.user_firstname' )->set(false),
        Field::inst( 'user.user_lastname' )->set(false),
        Field::inst( 'upvotes.crtdate' )->set(false)
            ->getFormatter( function ( $val, $data, $opts ) {
                return date('d.m.y H:i', $val);
            }),
        Field::inst( 'upvotes.upvotes' )->set(false),
        Field::inst( 'upvotes.downvotes' )->set(false)
    )
    ->leftJoin( 'businesscontact', 'businesscontact.id', '=', 'upvotes.customer' )
    ->leftJoin( 'user', 'user.id', '=', 'upvotes.crtuser' )
    ->process( $_POST )
    ->json();
