...
Editor::inst( $db, 'shipments' )
    ->debug( true )
    ->fields(
        Field::inst( 'shipments.id' )->set(false),
        Field::inst( 'collectiveinvoice.number' )->set(false),
        Field::inst( 'businesscontact.name1' )->set(false),
        Field::inst( 'shipments.parcelNumber' )->set(false),
        Field::inst( 'shipments.shipDate' )->set(false)
            ->getFormatter( function ( $val, $data, $opts ) {
                return date('d.m.Y', $val);
            })
    )
    ->leftJoin( 'collectiveinvoice', 'collectiveinvoice.id', '=', 'shipments.colinv' )
    ->leftJoin( 'businesscontact', 'businesscontact.id', '=', 'collectiveinvoice.businesscontact' )
    ->process( $_POST )
    ->json();
