...
$pdf->Cell(0, 0, $_LANG->get("gerne unterbreiten wir Ihnen nachfolgendes Angebot:"), 0, 1);
$pdf->Ln();
$pdf->Ln();
...
foreach ($calcs as $calc) {
    ...
    if(count($calc->getPositionsForDocuments()) > 0 && $calc->getPositionsForDocuments() != false){
        foreach ($calc->getPositionsForDocuments() AS $pos){
            ...
            if($pos->getShowPrice() == 1){
                $pdf->Cell(0, 0, printPrice($pos->getCalculatedPrice())." ".$_USER->getClient()->getCurrency()."", 0, 1, 'R');
            }
        }
        $pdf->Ln();
        $pdf->Ln();
    ...
    $pdf->Ln();
    $pdf->SetFont($font, 'b', 10);
    $pdf->Cell($tsize, 0, "Preis netto:", 0, 1);
    $pdf->SetFont($font, '', 10);
...
$footer3 .= $_LANG->get("Es gelten die allgemeinen Gesch&auml;ftsbedingungen der {$_USER->getClient()->getName()}", 0, 'L');

$pdf->MultiCell(0, 0, $footer1, 0, 'L');
$pdf->Ln();
$pdf->MultiCell(0, 0,$footer2, 0, 'L');
$pdf->MultiCell(0, 0,$footer3, 0, 'L');
$pdf->Ln();
