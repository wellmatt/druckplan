...
if ($perso->getLineByLine() == 0) {
	...
	foreach ($all_items as $oitem){
        ...
        if ($item->getBoxtype() == 1){
            ...
            if ($tabin === false){
                $pdf->Cell($width, $height, $oitem->getValue(), 0, 0, $justification, false , '', 0, true, 'T', 'B');
            } else {
                ...
                if ($item->getDependencyID() == 0) {    // erstes fix object links
                    ...
                    foreach ($all_items as $suboitem){
                        ...
                        if ($subitem->getGroup() == $group && $suboitem->getValue() != "") {
                            if ($subitem->getDependencyID() == $item->getId()) { // alle nicht fix objebte dieser Abh�nigkeit
                                ...
                                if ($subitem->getTitle() == "spacer"){
                                    ...
                                    if ($subitem->getBoxtype() == 1){
                                        ...
                                        if ($tabin === false){
                                            $pdf->Cell($subwidth, $subheight, $suboitem->getValue(), 0, 0, $justification, false , '', 0, true, 'T', 'B');
                                        } else {
