...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (@file_exists($outpath.$font_name.'.php')) {
			echo "this font already exist (delete it from fonts folder to rebuild it)</br>";
			return $font_name;
		}
		...
		if (empty($fonttype)) {
			if (TCPDF_STATIC::_getULONG($font, 0) == 0x10000) {
				...
				echo "Unsupported font format: OpenType with CFF data</br>";
				return false;
			} else {
				// Type 1
				$fonttype = 'Type1';
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($a['marker'] != 128) {
				echo "Font file is not a valid binary Type1</br>";
				return false;
			}
			...
			if (TCPDF_STATIC::_getULONG($font, $offset) != 0x10000) {
				echo "sfnt version must be 0x00010000 for TrueType version 1.0.</br>";
				return false;
			}
			$offset += 4;
			...
			if (TCPDF_STATIC::_getULONG($font, $offset) != 0x5F0F3CF5) {
				echo "magicNumber must be 0x5F0F3CF5</br>";
				return false;
			}
			$offset += 4;
			$offset += 2; // skip flags
			...
			$offset += 2;
			if ($fsType == 2) {
				echo "This Font cannot be modified, embedded or exchanged in any manner without first obtaining permission of the legal owner.</br>";
				return false;
			}
			// ---------- get font name ----------
			$fmetric['name'] = '';
